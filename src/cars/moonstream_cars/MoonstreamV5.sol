...
contract MoonstreamV5 is ICar {
    struct Cars {
        Monaco.CarData[] allCars;
        Monaco.CarData ourCar;
        uint256 ourCarIndex;
        uint256 projectedSpeed;
        ...
        if (cars.ourCarIndex != 0) {
            ...
            for (uint i = 0; i < bananas.length; ++i) {
                ...
                if (bananas[i] <= cars.leadCar.y) {
                    effectiveness = (cars.projectedSpeed / 4);
                    bananaInTheWay = true;
                    break;
                }
            }

            if (!bananaInTheWay && cars.ourCarIndex == 1) {
                uint256 leadDistance = cars.leadCar.y - cars.ourCar.y;
                if (
                    cars.leadCar.shield == 0 &&
                    (cars.leadCar.y - cars.ourCar.y) > 0
                ) {
    ...
    function getBananaEffectiveness(
        ...
        if (cars.ourCarIndex == 0) {
            ...
            uint256 lagSpeedGap = cars.projectedSpeed - cars.lagCar.speed;
            if (lagDistance > 0) {
                if (lagSpeedGap < 0) {
                    effectiveness += 2;
                }
                effectiveness += (cars.lagCar.speed / 4);
            }
        }
        if (cars.ourCarIndex == 1 && cars.ourCar.y > cars.lagCar.y) {
            effectiveness += 1;
        }
    }
        ...
        if (cars.ourCarIndex != 0 && cars.allCars[0].y > cars.ourCar.y) {
            ...
            if (cars.leadCar.y > cars.ourCar.y) {
                for (uint i = 0; i < bananas.length; ++i) {
                    ...
                    // If a banana is on top of the closest car, the banana is hit
                    if (bananas[i] <= cars.leadCar.y) {
                        effectiveness += cars.projectedSpeed / 2;
                    }
                }
            }
        }
    ...
    function isShieldEfficient(
        ...
        return shieldCost < scaleTargetCost(efficiencyCost, scalerWad);
    }
    ...
    function projectAcceleration(
        ...
        uint256 accelCount = 1;
        uint256 totalCost = monaco.getAccelerateCost(accelCount);
        uint256 marginalCost = totalCost;
        while (
            marginalCost < targetAccelPrice &&
            hasEnoughBalance(cars.ourCar, totalCost)
        ) {
            uint256 oldTotal = totalCost;
            accelCount++;
            totalCost = monaco.getAccelerateCost(accelCount);
            marginalCost = totalCost - oldTotal;
        }
        return accelCount - 1;
    }
    ...
    function optimalAccelerate(
        ...
        accelerate(
            monaco,
            cars.ourCar,
            projectAcceleration(monaco, cars, scalerWad)
        );
    }
    ...
    function takeYourTurn(
        ...
        Cars memory cars = Cars({
            ...
            projectedSpeed: allCars[ourCarIndex].speed,
            leadCar: allCars[0],
            lagCar: allCars[2]
        });
        ...
        if (cars.ourCar.speed == 0) {
            if (
                monaco.getAccelerateCost(1) < 3 * targetAccelPrice &&
                hasEnoughBalance(cars.ourCar, monaco.getAccelerateCost(1))
            ) {
                accelerate(monaco, cars.ourCar, 1);
                cars.ourCar.speed = 1;
        ...
        if (cars.ourCar.speed == 1) {
            if (
                monaco.getAccelerateCost(1) < 2 * targetAccelPrice &&
                hasEnoughBalance(cars.ourCar, monaco.getAccelerateCost(1))
            ) {
                accelerate(monaco, cars.ourCar, 1);
                cars.ourCar.speed = 2;
        ...
        // cars.projectedSpeed =
        //     cars.ourCar.speed +
        //     projectAcceleration(monaco, cars, spendingScalerWad);
        if (
            isSuperEfficient(
                baseCosts.superCost,
