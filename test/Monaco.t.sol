...
import {MoonstreamV5} from "../src/cars/moonstream_cars/MoonstreamV5.sol";
...
contract MonacoTest is Test {
    ...
    function testGames() public {
        ...
        vm.writeFile(
            string.concat("logs/", vm.toString(address(w1)), ".csv"),
            "turns,balance,speed,y,shield\n"
        );
        vm.writeFile(
            string.concat("logs/", vm.toString(address(w2)), ".csv"),
            "turns,balance,speed,y,shield\n"
        );
        vm.writeFile(
            string.concat("logs/", vm.toString(address(w3)), ".csv"),
            "turns,balance,speed,y,shield\n"
        );
        vm.writeFile(
            "logs/prices.csv",
            "turns,accelerateCost,shellCost,superShellCost,shieldCost\n"
        );
        vm.writeFile(
            "logs/sold.csv",
            "turns,acceleratesBought,shellsBought,superShellBought,shieldsBought\n"
        );
        ...
        while (monaco.state() != Monaco.State.DONE) {
            ...
            currentTurn.currentCar = address(
                monaco.cars(monaco.turns() % CAR_LEN)
            );

            // cache the current # abilities sold
            for (
                uint256 abilityIdx = 0;
                abilityIdx <= uint256(Monaco.ActionType.SHIELD);
                ++abilityIdx
            ) {
                currentTurn.usedAbilities[abilityIdx] = monaco.getActionsSold(
                    Monaco.ActionType(abilityIdx)
                );
            }
            ...
            for (
                ...
                currentTurn.usedAbilities[abilityIdx] =
                    monaco.getActionsSold(Monaco.ActionType(abilityIdx)) -
                    currentTurn.usedAbilities[abilityIdx];
            }
            ...
            for (uint256 i = 0; i < allCarData.length; i++) {
                ...
                for (
                    ...
                    currentTurn.bought[abilityIdx] = monaco.getActionsSold(
                        Monaco.ActionType(abilityIdx)
                    );
                }
                ...
                vm.writeLine(
                    ...
                    string.concat(
                        ...
                        ",",
                        vm.toString(
                            monaco.getActionsSold(Monaco.ActionType.ACCELERATE)
                        ),
                        ",",
                        vm.toString(
                            monaco.getActionsSold(Monaco.ActionType.SHELL)
                        ),
                        ",",
                        vm.toString(
                            monaco.getActionsSold(Monaco.ActionType.SUPER_SHELL)
                        ),
                        ",",
                        vm.toString(
                            monaco.getActionsSold(Monaco.ActionType.SHIELD)
                        )
                    )
                );
            }
        ...
        emit log_named_address(
            "\nWinner",
            address(monaco.getAllCarData()[0].car)
        );
    ...
    function getAbilityCost(uint256 abilityIdx) private view returns (uint256) {
